...
char gammamsg[5 + 4][26 + 2] = {GAMMALVL0,  GAMMALVL05, GAMMALVL1,
                                GAMMALVL15, GAMMALVL2,  GAMMALVL25,
...
static void M_ChangeSensitivity_x2(
    int choice);  // [crispy] mouse sensitivity menu
static void M_ChangeSensitivity_y(
    int choice);                        // [crispy] mouse sensitivity menu
...
static menu_t Crispness1Def = {
    crispness1_end, &OptionsDef, Crispness1Menu, M_DrawCrispness1, 48, 28, 1};
...
static menu_t Crispness2Def = {
    crispness2_end, &OptionsDef, Crispness2Menu, M_DrawCrispness2, 48, 28, 1};
...
static menu_t Crispness3Def = {
    crispness3_end, &OptionsDef, Crispness3Menu, M_DrawCrispness3, 48, 28, 1};
...
static menu_t Crispness4Def = {
    crispness4_end, &OptionsDef, Crispness4Menu, M_DrawCrispness4, 48, 28, 1};
...
void M_DrawOptions(void)
{
  ...
  */
  ...
  // [crispy] no patches are drawn in the Options menu anymore
  /*
      V_DrawPatch(OptionsDef.x + 120, OptionsDef.y + LINEHEIGHT *
     messages, W_CacheLumpName(DEH_String(msgNames[showMessages]), PU_CACHE));
  */
  M_WriteText(
  ...
  M_DrawThermo(OptionsDef.x, OptionsDef.y + LINEHEIGHT * (scrnsize + 1), 9 + 3,
...
{
  M_snprintf(crispy_menu_text, sizeof(crispy_menu_text), "%s%s", crstr[CR_GOLD],
             item);
...
static void M_DrawCrispnessMultiItem(int y,
                                     char* item,
                                     multiitem_t* multiitem,
                                     int feat,
                                     boolean cond)
{
  M_snprintf(crispy_menu_text, sizeof(crispy_menu_text), "%s%s: %s%s",
...
static void M_DrawCrispness1(void)
{
  ...
  M_DrawCrispnessItem(crispness_smoothscaling, "Smooth Pixel Scaling",
                      crispy->smoothscaling, true);
  M_DrawCrispnessMultiItem(crispness_aspectratio, "Force Aspect Ratio",
                           multiitem_aspectratio, aspect_ratio_correct, true);
  ...
  M_DrawCrispnessItem(crispness_coloredblood, "Colored Blood and Corpses",
                      crispy->coloredblood, true);
...
static void M_DrawCrispness2(void)
{
  ...
  M_DrawCrispnessItem(crispness_soundfix, "Misc. Sound Fixes", crispy->soundfix,
                      true);
  ...
  M_DrawCrispnessMultiItem(crispness_secretmessage, "Show Revealed Secrets",
...
static void M_DrawCrispness3(void)
{
  ...
  M_DrawCrispnessItem(crispness_weaponsquat, "Squat weapon down on impact",
  ...
  //  M_DrawCrispnessItem(crispness_extsaveg, "Extended Savegames",
  //  crispy->extsaveg, true);
...
static void M_DrawCrispness4(void)
{
  ...
  M_DrawCrispnessItem(crispness_recoil, "Weapon Recoil Thrust", crispy->recoil,
                      crispy->singleplayer);
...
void M_EndGameResponse(int key)
{
  ...
  // [crispy] killough 5/26/98: make endgame quit if recording or playing back
  // demo
...
static int G_ReloadLevel(void)
{
  ...
  if (gamestate == GS_LEVEL)
  {
    ...
    G_DeferedInitNew(gameskill, gameepisode,
                     demorecording ? startmap : gamemap);
    result = true;
  }
...
void M_Drawer(void)
{
  ...
  for (i = 0; i < max; i++)
  {
    ...
    if (name[0])  // && W_CheckNumForName(name) > 0) // [crispy] moved...
    {
      ...
      if ((currentMenu == &OptionsDef && i == endgame &&
           (!usergame || netgame)) ||
          (currentMenu == &MainDef && i == newgame &&
           (demorecording || (netgame && !demoplayback))))
        dp_translation = cr[CR_DARK];
  ...
  if (currentMenu == CrispnessMenus[crispness_cur])
  {
    ...
    M_WriteText(currentMenu->x - 8, currentMenu->y + CRISPY_LINEHEIGHT * itemOn,
                item);
    dp_translation = NULL;
  }
  else
