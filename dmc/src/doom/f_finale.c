...
void F_StartFinale(void)
{
  ...
  for (i = 0; i < arrlen(textscreens); ++i)
  {
    ...
    if (gameepisode == screen->episode && gamemap == screen->level)
    {
...
// [crispy] play attack sound based on state action function (instead of state
// number)
...
void F_CastTicker(void)
{
  ...
  else
  {
    ...
    */
    ...
    switch (st)
    {
      case S_PLAY_ATK2:	sfx = sfx_dshtgn; break; // [crispy] fix Doomguy
   in casting sequence case S_POSS_ATK2:	sfx = sfx_pistol; break; case
   S_SPOS_ATK2:	sfx = sfx_shotgn; break; case S_VILE_ATK2:	sfx =
   sfx_vilatk; break; case S_SKEL_FIST2:	sfx = sfx_skeswg; break; case
   S_SKEL_FIST4:	sfx = sfx_skepch; break; case S_SKEL_MISS2:	sfx =
   sfx_skeatk; break; case S_FATT_ATK8: case S_FATT_ATK5: case S_FATT_ATK2:
   sfx = sfx_firsht; break; case S_CPOS_ATK2: case S_CPOS_ATK3: case
   S_CPOS_ATK4:	sfx = sfx_shotgn; break; case S_TROO_ATK3:	sfx =
   sfx_claw; break; case S_SARG_ATK2:	sfx = sfx_sgtatk; break; case
   S_BOSS_ATK2: case S_BOS2_ATK2: case S_HEAD_ATK2:	sfx = sfx_firsht; break;
...
boolean F_CastResponder(event_t* ev)
{
  ...
  if (xdeath && mobjinfo[castorder[castnum].type].xdeathstate)
    ...
    S_StartSound(
        NULL, F_RandomizeSound(mobjinfo[castorder[castnum].type].deathsound));
