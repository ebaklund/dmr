...
static int S_AdjustSoundParams(mobj_t* listener,
                               mobj_t* source,
                               int* vol,
                               int* sep)
{
...
void S_StartSound(void* origin_p, int sfx_id)
{
  ...
  if (origin && origin != players[consoleplayer].mo &&
      origin != players[consoleplayer].so)  // [crispy] weapon sound source
  {
    ...
    if (c->sfxinfo)
    {
      if (I_SoundIsPlaying(c->handle))
      {
        ...
        if (c->origin && listener != c->origin &&
            c->origin !=
                players[consoleplayer].so)  // [crispy] weapon sound source
        {
...
void S_ChangeMusic(int musicnum, int looping)
{
      ...
      W_CheckNumForName("D_INTROA") >= 0)
  {
    ...
    // [crispy] if D_INTRO is from a PWAD, and D_INTROA is from a different WAD
    // file, play the former
  ...
  if (musicnum <= mus_None || musicnum >= NUMMUSIC ||
      (musicnum >= mus_runnin) || S_music[musicnum].lumpnum == -1)
  {
