...
void P_LoadSegs(int lump)
{
  ...
  {
    li->v1 =
        &vertexes[(unsigned short)SHORT(ml->v1)];  // [crispy] extended nodes
    li->v2 =
        &vertexes[(unsigned short)SHORT(ml->v2)];  // [crispy] extended nodes

    li->angle = (SHORT(ml->angle)) << FRACBITS;
    //	li->offset = (SHORT(ml->offset))<<FRACBITS; // [crispy] recalculated
    //below
    ...
    {
      I_Error(
          "P_LoadSegs: linedef %d for seg %d references a non-existent sidedef "
          "%d",
    ...
    if (ldef->flags & ML_TWOSIDED)
    {
      ...
      if (sidenum < 0 || sidenum >= numsides)
      {
        ...
        if (li->sidedef->midtexture)
        {
          li->backsector = 0;
          fprintf(stderr,
                  "P_LoadSegs: Linedef %d has two-sided flag set, but no "
                  "second sidedef\n",
                  i);
        }
        else
...
void P_LoadNodes(int lump)
{
  ...
  for (i = 0; i < numnodes; i++, no++, mn++)
  {
    ...
    {
      no->children[j] =
          (unsigned short)SHORT(mn->children[j]);  // [crispy] extended nodes
...
void P_LoadLineDefs(int lump)
{
  ...
  for (i = 0; i < numlines; i++, mld++, ld++)
  {
    ...
    v1 = ld->v1 =
        &vertexes[(unsigned short)SHORT(mld->v1)];  // [crispy] extended nodes
    v2 = ld->v2 =
        &vertexes[(unsigned short)SHORT(mld->v2)];  // [crispy] extended nodes
    ld->dx = v2->x - v1->x;
    ld->dy = v2->y - v1->y;
      ...
      do
      {
        ...
        if (!v->moved)
        {
          ...
          if (v != l->v1 && v != l->v2)
          {
            ...
            v->r_x = (fixed_t)(
                (dx2 * v->x + dy2 * l->v1->x + dxy * (v->y - l->v1->y)) / s);
            v->r_y = (fixed_t)(
                (dy2 * v->y + dx2 * l->v1->y + dxy * (v->x - l->v1->x)) / s);
        ...
        // [crispy] if v doesn't point to the second vertex of the seg already,
        // point it there
