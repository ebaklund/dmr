...
void HU_Init(void)
{
  ...
  for (i = 0; laserpatch[i].c; i++)
  {
    ...
    // [crispy] check for alternative crosshair patches from e.g.
    // prboom-plus.wad first
...
static void HU_SetSpecialLevelName(const char* wad, const char** name)
{
  ...
  for (i = 0; i < arrlen(speciallevels); i++)
  {
    ...
    if (gamemission == speciallevel.mission &&
...
void HU_Start(void)
{
  ...
  // [crispy] create the generic map title, kills, items, secrets and level time
  // widgets
...
//  V_DrawHorizLine(0, SCREENHEIGHT - 2, 1, colormaps[4]); // [crispy] white
//  start V_DrawHorizLine(i - 1, SCREENHEIGHT - 2, 1, colormaps[4]); // [crispy]
//  white end
#endif
}
...
static void HU_DrawCrosshair(void)
{
  ...
  if (weaponinfo[plr->readyweapon].ammo == am_noammo ||
      plr->playerstate != PST_LIVE || automapactive || menuactive || paused ||
      secret_on)
    return;
  ...
  //  V_DrawHorizLine(0, (screenblocks <= 10) ? (SCREENHEIGHT/2-ST_HEIGHT) :
  //  (SCREENHEIGHT/2), SCREENWIDTH, 128);
}
...
void HU_Ticker(void)
{
      ...
      (automapactive && crispy->automapstats == WIDGETS_AUTOMAP))
  {
    ...
    if (extrakills)
      M_snprintf(str, sizeof(str), "%s%s%s%d/%d+%d", cr_stat, kills,
                 crstr[CR_GRAY], plr->killcount, totalkills, extrakills);
    else
      M_snprintf(str, sizeof(str), "%s%s%s%d/%d", cr_stat, kills,
                 crstr[CR_GRAY], plr->killcount, totalkills);
      ...
      (automapactive && crispy->leveltime == WIDGETS_AUTOMAP))
  {
    ...
    if (time >= 3600)
      ...
      M_snprintf(str, sizeof(str), "%s%02d:%02d", crstr[CR_GRAY], time / 60,
                 time % 60);
  ...
  if (plr->powers[pw_showfps])
  {
    M_snprintf(str, sizeof(str), "%s%-4d %sFPS", crstr[CR_GRAY], crispy->fps,
               cr_stat2);
