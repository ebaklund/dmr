...
//	dscount++;
  ...
  */
  ...
  do
  {
    ...
    // [crispy] fix flats getting more distorted the closer they are to the
    // right
...
void R_FillBackScreen(void)
{
  ...
  for (x = 0; x < (scaledviewwidth >> crispy->hires); x += 8)
    V_DrawPatch((viewwindowx >> crispy->hires) + x,
  ...
  for (y = 0; y < (viewheight >> crispy->hires); y += 8)
    V_DrawPatch(
        (viewwindowx >> crispy->hires) + (scaledviewwidth >> crispy->hires),
        (viewwindowy >> crispy->hires) + y, patch);
