...
// [crispy] distinguish classic status bar with background and player face from
// Crispy HUD
...
// [crispy] pseudo cheats to eat up the first digit typed after a cheat
...
boolean ST_Responder(event_t* ev)
{
  ...
  else if (ev->type == ev_keydown)
  {
    if (!netgame && gameskill != sk_nightmare)
    {
        ...
        else
        {
          ...
          */
          ...
          else
          {
            ...
            // [crispy] eat key press, i.e. don't change weapon upon music
            // change
            return true;
          }
        }
      }
      // [crispy] eat up the first digit typed after a cheat expecting two
      // parameters
      ...
      else if (cht_CheckCheatSP(&cheat_noclip, ev->data2) ||
               ...
               cht_CheckCheatSP(&cheat_massacre3, ev->data2))
      {
        ...
        const char* const monster = "Monster";
      ...
      // [crispy] implement "nomomentum" cheat, ne debug aid -- pretty useless,
      // though
      ...
      else if (cht_CheckCheatSP(&cheat_weapon, ev->data2))
      {
        ...
        // [crispy] TNTWEAP0 takes away all weapons and ammo except for the
        ...
        if (!plyr->message)
        {
          M_snprintf(msg, sizeof(msg), "Weapon %s%d%s %s", crstr[CR_GOLD],
                     w + 1, crstr[CR_NONE],
...
#undef BUILD_DATE
        ...
        !menuactive)  // [crispy] prevent only half the screen being updated
    {
      ...
      if (P_GetNumForMap(epsd, map, false) >= 0)
      {
        ...
        // [crisp] allow IDCLEV during demo playback and warp to the requested
        // map
        if (demoplayback)
        {
          if (map > gamemap)
...
void ST_updateWidgets(void)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    if (plyr->tryopen[i])
    {
      ...
      {
        keyboxes[i] =
            (plyr->tryopen[i] & KEYBLINKMASK) ? i + st_keyorskull[i] : -1;
      }
...
void ST_doPaletteStuff(void)
{
  ...
  if (cnt)
  {
    ...
    palette += STARTREDPALS;
           ...
           plyr->health > 0)  // [crispy] never show the yellow bonus palette
                              // for a dead player
  {
...
static byte* ST_WidgetColor(int i)
{
  ...
  switch (i)
  {
    ...
    case hudcolor_health:
    {
      ...
      // [crispy] Invulnerability powerup and God Mode cheat turn Health values
      // gray
    ...
    case hudcolor_armor:
    {
      // [crispy] Invulnerability powerup and God Mode cheat turn Armor values
      // gray
      if (plyr->cheats & CF_GODMODE || plyr->powers[pw_invulnerability])
        return cr[CR_GRAY];
...
void ST_drawWidgets(boolean refresh)
{
  ...
  if (st_crispyhud)
  {
    ...
    if (plyr->readyweapon == wp_fist && plyr->powers[pw_strength])
    {
      ...
      if (!patch)
      {
        ...
        // [crispy] should you ever play with the IDBEHOLDS cheat and the
        // Shareware version...
        else
        {
  ...
  if (st_crispyhud && screenblocks == CRISPY_HUD)
    V_CopyRect(ST_FX, 1, st_backing_screen, SHORT(faceback->width),
               ST_HEIGHT - 1, ST_FX, ST_Y + 1);
...
void ST_Drawer(boolean fullscreen, boolean refresh)
{
  ...
  // from Crispy HUD
  st_crispyhud =
      screenblocks >= CRISPY_HUD && (!automapactive || crispy->automapoverlay);
  st_classicstatusbar = st_statusbaron && !st_crispyhud;
  st_statusbarface =
      st_classicstatusbar || (st_crispyhud && screenblocks == CRISPY_HUD);
  ...
  if (st_firsttime)  // If just after ST_Start(), refresh all
    ST_doRefresh();
  else  // Otherwise, update as little as possible
    ST_diffDraw();
...
void ST_Init(void)
{
  ...
  st_backing_screen = (pixel_t*)Z_Malloc(
      (ST_WIDTH << 1) * (ST_HEIGHT << 1) * sizeof(*st_backing_screen),
      PU_STATIC, 0);
}
...
void ST_DrawDemoTimer(const int time)
{
  ...
  n = M_snprintf(buffer, sizeof(buffer), "%02i %02i %02i", secs / 60, secs % 60,
                 time % TICRATE);
